...
export default function App() {
  ...
  const handleAnalyze = async (overrideText?: string, overrideSender?: string) => {
    ...
    try {
      ...
      let friendlyReason = 'Analysis generated by ScamShield on-device heuristic engine.';
      if (rawError.includes('Failed to fetch') || rawError.includes('NetworkError') || rawError.includes('aborted') || !navigator.onLine) {
        friendlyReason = 'Live cloud connection unavailable or offline. ScamShield automatically activated on-device heuristic security models to complete your analysis.';
      } else if (rawError) {
        friendlyReason = rawError;
      }
      ...
      // Smooth scroll to results on fallback
      setTimeout(() => {
        const el = document.getElementById('inspection-results-container');
        if (el) {
          el.scrollIntoView({ behavior: 'smooth', block: 'start' });
          ...
          <section id="inspection-results-container" className="space-y-6 pt-2">
            ...
            {analysis.isFallbackRuleEngine && (
              <div
                ...
                className="rounded-2xl border border-amber-500/40 bg-amber-950/30 p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs font-mono-code text-amber-200 shadow-lg"
              >
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-bold text-amber-300 text-sm tracking-tight">
                        Zero-Downtime Heuristics Active
                      </span>
                    ...
                    <p className="text-xs font-sans text-amber-200/90 leading-relaxed max-w-2xl">
                      {analysis.fallbackReason || 'High-accuracy rule heuristics used for instant zero-downtime protection.'}
                    </p>
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 active:bg-amber-500/40 border border-amber-500/50 text-amber-200 text-xs font-sans font-semibold tracking-wide transition-all shrink-0 self-start sm:self-auto cursor-pointer"
                >
                  <RefreshCw className="w-3.5 h-3.5" />
                  <span>Retry with Cloud AI</span>
                </button>
              </div>
            )}
